...
all.genes <- read_tsv("/projects/munger-lab/projects/DO_mNPC/ESC_NPC_eQTL_comparison/_data/ENSMUSGid_to_symbol_v84.txt") # gene.info
...
# Then, I identify peaks that are further apart than 5Mb +
# I will seperate the NPC peaks and add as rows to be included in the matching later
# esc_rna + npc_rna
merged.peaks.esc.npc <- full_join(peaks.esc_rna.wbp, peaks.npc_rna.wbp, 
                                  by=c("phenotype","peak_chr"), 
                                  suffix=c(".esc_rna",".npc_rna")) %>%
  mutate(bp_diff = abs(interp_bp_peak.esc_rna-interp_bp_peak.npc_rna)) %>% # there are QTL for the same gene on the same chr but further than the threshold, which should be added as a separate peak/row 
  mutate(match=ifelse(!is.na(bp_diff), TRUE,NA )) %>% 
  mutate(match=ifelse( match ==TRUE & bp_diff >10e06, FALSE,match)) %>% # using 10Mb at the moment
...
merged.peaks.esc.npc.filled <- add_lods(peaks  = merged.peaks.esc.npc.fixed,
...
# in some cases the difference between peak locations is more than 10Mb but the filled in lod score is still >5 for both eQTL.
# I think those should still be called 'shared'. not sure how to avoid duplication though. Because at this moment there will be 2 of each!
# I can inner join with ESC or NPC eQTL data frames to look at specific overlap! 
peaks.esc.npc.rna.wgeneinfo <- merged.peaks.esc.npc.filled %>%  
  ...
  mutate(
    match = case_when(
      ...
      match == TRUE ~ "shared",
      match == FALSE & ( lod.esc_rna >5 & lod.npc_rna >5 ) ~ "shared",
      match == FALSE & (lod.esc_rna > 5 & (lod.npc_rna < 5 | is.na(lod.npc_rna)) )  ~ "esc_rna",
      match == FALSE & (lod.npc_rna > 5 & (lod.esc_rna < 5 | is.na(lod.esc_rna)) )  ~ "npc_rna"
    )
  ) %>% 
...
peaks.esc.npc.rna$cumsum_bp_gene <- peaks.esc.npc.rna$midpoint + chrom_lens_offset[peaks.esc.npc.rna$gene_chr]
peaks.esc.npc.rna  <- peaks.esc.npc.rna  %>% arrange(peak_chr)

# separating ESC/NPC eQTL data frames to avoid duplication.
peaks.esc_rna.wnpc.overlap <- peaks.esc.npc.rna %>% 
  inner_join( peaks.esc_rna %>% 
                mutate( peak_cM = as.character(peak_cM)) %>% 
                select( ensembl_gene_id = phenotype, peak_chr, peak_cM.esc_rna = peak_cM, lod.esc_rna = lod))

peaks.npc_rna.wesc.overlap <- peaks.esc.npc.rna %>% 
  inner_join( peaks.npc_rna %>% 
                mutate( peak_cM = as.character(peak_cM)) %>% 
                select( ensembl_gene_id = phenotype, peak_chr, peak_cM.npc_rna = peak_cM, lod.npc_rna = lod))


# let's save the comparison tables. I can use them locally to summarize results in an Rnotebook. 
table.note <-paste0("This data has been prepared by SA using 01_Compare_DO_Peaks_onlyNPC_v2.R using the interactive que on the cluster on ", Sys.Date())
...
save(table.note, peaks.esc_rna.wnpc.overlap,peaks.npc_rna.wesc.overlap, file=paste0("/projects/munger-lab/projects/DO_mNPC/ESC_NPC_eQTL_comparison/_data/", "peaks_comparison_10Mb_ESC_NPC_v2.RData"))
