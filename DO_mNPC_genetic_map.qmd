...
Mediation analysis results are summarized in two notebooks where the first one is summarizing the full mediation results focusing on local and distant eQTL with best mediators being ESC or NPC transcripts.
Second notebook describes the DO mNPC eQTL hotspots, their characterization and mediation.
...
Only 23 have eQTLs in NPCs as listed below.
...
eqtl_npc_rna_meds_distal %>% 
  ...
  arrange( LOD) %>% 
  select( target_symbol, qtl_chr, gene_chr, mediator,qtl_lod, mediation_lod=LOD,  type) %>% 
  mutate( qtl_lod = round(qtl_lod,2),
