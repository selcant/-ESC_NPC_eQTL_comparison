...
# ggsave(npc_eqtl_map_plot, filename = here("figures","do_npc_eqtl_map.svg"), width = 8, height = 11, dpi = 300, units = "in")
npc_eqtl_map_plot
...
shared_eqtl_eff_plot <- shared_eqtl_effs_cor_df %>% 
  mutate( p_adj = p.adjust(p_val, method = "BH")) %>% 
...
# ggsave(shared_eqtl_eff_plot, filename = here("figures","shared_eqtl_eff_plot.svg"), width = 7, height = 4, units = "in", dpi = 300)

shared_eqtl_eff_plot
...
Csnk2a1_effs %>% 
  ...
  mutate( effect = factor(effect, levels = c("WSB","PWK","CAST","NZO","NOD","129","B6","AJ"))) |> 
  ggplot()+
  aes( x = effect,
       y = value, 
       col = type,
...
Csnk2a1_plot <- ggarrange( Csnk2a1_lod_plot, Csnk2a1_haplotype_plot, widths = c(1, 0.5))

Csnk2a1_plot
# ggsave(Csnk2a1_plot, filename = here("figures","Csnk2a1_plot.svg"), width = 9, height = 4, units = "in", dpi = 300)
...
amph_plot <- ggarrange( Amph_lod_plot, Amph_haplotype_plot, widths = c(1, 0.5))

amph_plot

# ggsave(amph_plot, filename = here("figures","amph_plot.svg"), width = 9, height = 4, units = "in", dpi = 300)
...
load(here("_data","do_mesc_expr_tpm03_06102024.RData"))#expr_tpm03.esc_rna, exprZ_tpm03.esc_rna
...
chr1_pc1_med_esc <- mediation.scan(target= chr1_pc1_rankZ[shared.samples,"PC1"],
                            mediator = exprZ_tpm03.esc_rna[shared.samples,intersect(all.genes$ensembl_gene_id, colnames(exprZ_tpm03.esc_rna)),drop=F],
                            annotation = all.genes %>%
                                  filter(ensembl_gene_id %in% colnames(exprZ_tpm03.esc_rna)) |> 
                                  mutate(chr=gene_chr,pos=abs(gene_end+gene_start)/2),
...
get_pcor_for_meds <- function(x, esc=FALSE, random = FALSE){
  ...
  if(esc == TRUE){
    gene_pcor_table <- tibble(
      ...
      !!gene := exprZ_tpm03.esc_rna[shared.samples,gene_dets$ensembl_gene_id],
...
cand_mediators_npc <- npc.genes |> 
  filter( 
    gene_chr == 1, 
    midpoint >= filter( bands.esc.npc.rna,chr==1)$start[1]-10e06, 
    midpoint  <= filter( bands.esc.npc.rna,chr==1)$end[1]+10e06 
    )

cand_mediators_esc <- all.genes |>
  filter( ensembl_gene_id %in% colnames(exprZ_tpm03.esc_rna)) |> 
  filter( 
    gene_chr == 1, 
    midpoint >= filter( bands.esc.npc.rna,chr==1)$start[1]-10e06 , 
...
pcor_results_merged |> 
  select(mediator, pcor, p_value, type) |> 
  arrange( desc(abs(pcor))) |> 
  mutate_if(is.numeric, formatC, digits = 2) |> 
  create_dt()
...
chr1_pc1_as_npc <- scan1snps(genoprobs = probs.npc_rna,
                     map = pmap_Mbp,
                     pheno = chr1_pc1_rankZ,
                     kinship = kinship_loco.npc_rna[[chr1_pc1_peak$chr]],
                     addcovar = covar.npc_rna,
                     chr = as.character(chr1_pc1_peak$chr),
                     query_func = query_variants,
                     start = chr1_pc1_peak$pos/1e06 - 5,
                     end = chr1_pc1_peak$pos/1e06 + 5,
                     keep_all_snps = F
                     )

genes <- query_genes( chr1_pc1_peak$chr,
                      chr1_pc1_peak$pos/1e06 - 5,
                      chr1_pc1_peak$pos/1e06 + 5) %>%
  distinct()
...
plot(chr1_pc1_as_npc$lod,
     chr1_pc1_as_npc$snpinfo,
     drop_hilit = 1,
...
Note that in the DO mESCs, *Rnf152* gets filtered out because it has a median tpm of ~0.3 which is below our threshold of 0.5. I went back and got the expression data to do eQTL mapping and mediation to have a point of comparison to the NPC results.  
...
# LOD plot
rnf152_esc_scan <- scan1( pheno = exprZ_tpm03.esc_rna[shared.samples,rnf152_eqtl$ensembl_gene_id,drop=FALSE],
...
rnf152_npc_scan %>% 
  ...
  full_join(
    rnf152_esc_scan |> 
      as_tibble(rownames = "marker") |> 
      rename( esc_rna = rnf152_eqtl$ensembl_gene_id) 
  ) |> 
  left_join(map_dat2) |> 
  mutate( esc_rna = as.numeric(esc_rna),
          npc_rna = as.numeric(npc_rna)) -> rnf152_qtl_scans
...
# Effects plot
rnf152_npc_eff_scan <- scan1blup( pheno = exprZ.npc_rna[shared.samples,rnf152_eqtl$ensembl_gene_id,drop=FALSE], 
                           genoprobs = shared.probs.npc_rna[,rnf152_eqtl$peak_chr], 
                           kinship = kinship.shared.npc_rna[[rnf152_eqtl$peak_chr]],
                           addcovar = covar.npc_rna[shared.samples,]
  
)

rnf152_esc_eff_scan <- scan1blup( pheno = exprZ_tpm03.esc_rna[shared.samples,rnf152_eqtl$ensembl_gene_id,drop=FALSE], 
                           genoprobs = shared.probs.esc_rna[,rnf152_eqtl$peak_chr], 
                           kinship = kinship.shared.esc_rna[[rnf152_eqtl$peak_chr]],
                           addcovar = covar.esc_rna[shared.samples,]
  
)
# find markers before/after the NPC eQTL peak to look at the same locus in the ESC scan
rnf152_npc_eqtl_closest_marker <- map_dat2 |> 
  filter( chr == rnf152_eqtl$peak_chr,
          abs(pos_bp-rnf152_eqtl$interp_bp_peak.npc_rna) == 0
  )
rnf152_eqtl_markers <- map_dat2 |> 
  filter( n == rnf152_npc_eqtl_closest_marker$n-1 |
            n == rnf152_npc_eqtl_closest_marker$n |
            n == rnf152_npc_eqtl_closest_marker$n + 1)

#rnf152_esc_eff <- colMeans(rnf152_esc_eff_scan[rnf152_eqtl_markers$marker,LETTERS[1:8]]) 
rnf152_esc_eff <- (rnf152_esc_eff_scan[rnf152_npc_eqtl_closest_marker$marker,LETTERS[1:8]]) 
names(rnf152_esc_eff) <- paste0(names(rnf152_esc_eff),".esc_rna")

#rnf152_npc_eff <- colMeans(rnf152_npc_eff_scan[rnf152_eqtl_markers$marker,LETTERS[1:8]]) 
rnf152_npc_eff <- (rnf152_npc_eff_scan[rnf152_npc_eqtl_closest_marker$marker,LETTERS[1:8]]) 
names(rnf152_npc_eff) <- paste0(names(rnf152_npc_eff),".npc_rna")
