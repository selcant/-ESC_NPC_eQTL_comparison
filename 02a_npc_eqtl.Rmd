...
There are more than a 1000 results for TF motif overrepresentation! Is there a common list of genes that are driving this? Let's look at which genes are being identified and their intersection.
...
Lats1_esc_scan %>% 
  as.data.frame( ) %>% 
  rename( esc_rna = Lats1_eqtl$ensembl_gene_id) %>% 
  mutate( marker = dimnames(Lats1_esc_scan)[[1]]) %>% 
  left_join(map_dat2) %>% 
  cbind(
    Lats1_npc_scan %>% as.data.frame() %>% rename( npc_rna = Lats1_eqtl$ensembl_gene_id)
  ) -> Lats1_qtl_scans
...
# Effects plot
Lats1_esc_eff <- peaks.esc_rna.wEffs %>% 
  inner_join( Lats1_eqtl %>% select(ensembl_gene_id, peak_chr, lod.esc_rna))

Lats1_npc_eff <- peaks.npc_rna.wEffs %>% 
  inner_join( Lats1_eqtl %>% select(ensembl_gene_id, peak_chr, lod.npc_rna))
Lats1_effs <- Lats1_esc_eff %>% 
  select( mgi_symbol, paste0(LETTERS[1:8], ".esc_rna")) %>% 
  cbind( Lats1_npc_eff %>% select(paste0(LETTERS[1:8], ".npc_rna")) ) 
