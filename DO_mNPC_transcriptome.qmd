...
I used Wilcoxon rank sum test to identify differentially expressed genes in mNPCs in contrast to mESCs and functionally characterized up and down regulated genes using ORA.
...
Looking at the highest 5% contributing genes to PC1 with positive and negative loadings.
...
PC1 drivers with positive loadings are over-represented for:
...
-   TF motifs: OTX2, PITX1, PITX2, c-MYC, MXI1, E4BP4, HFH8, PITX3, ZFP536, SAP1A, ARNT, 



PC1 drivers with negative loadings are over-represented for:
...
-   Genes involved in autophagy and neuron apoptotic process

-   Genes found in axons.

-   TF motifs: MAFA, NRF2, MATH1, c-ETS2, TFEA

-   Many human phenotypes, including neuronal ones. 
...
spindle_genes <- g.pc1_top$result %>% 
  filter( p_value <0.01) %>% 
...
pca_esc_npc$x |> 
  ...
  mutate(sex = ifelse(sex == "F", "Female", "Male")) %>%
  GGally::ggpairs(
    columns = 3:12, progress = FALSE, ggplot2::aes(color = sex, shape = type),
    upper = list(continuous = "density", combo = "box_no_facet"),
    lower = list(continuous = "points", combo = "dot_no_facet")
  ) + 
  theme_pubclean(base_size = 14) +
