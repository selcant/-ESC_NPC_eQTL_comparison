...
# ggsave(pca_plot, filename = here("figures","figure1c.svg"), width = 8, height = 4, dpi = 300, units = "in")
...
PC4 is mostly driven by genes on sex chromosomes.
...
gsva_rna_esc_npc_results %>% 
  group_by( Category) %>% 
  rstatix::anova_test( Enrichment_Score ~ type+sex+lifr_geno) %>% 
  rstatix::adjust_pvalue( method = "BH") %>%
  rstatix::add_significance("p.adj") %>% 
...
gsva_rna_esc_npc_results %>% 
  group_by(Category) %>% 
  rstatix::tukey_hsd( Enrichment_Score ~ type+sex+lifr_geno) %>% 
  ungroup() %>% 
  as_tibble() %>% 
...
gsva_rna_esc_npc_tukey %>%
  ...
  rbind(
    ...
    rbind(
    gsva_rna_esc_npc_tukey %>%
      filter( Category %in% (signif_results_tukey_rna_esc_npc %>% filter( p.adj <= 0.01, term =="lifr_geno"))$Category, term == "lifr_geno" )
...
Let's simplify the GO BP terms across effects:

```{r esc_npc_gsva_go_bp}
#| warning: false
#| message: false
#| fig-width: 21
#| fig-height: 8
...
gsva_esc_npc_go_bp <- list(
  Sex = unique((signif_results_tukey_rna_esc_npc %>% filter( p.adj <= 0.01, term =="sex"))$Category),
  Cell_state = unique((signif_results_tukey_rna_esc_npc %>% filter( p.adj <= 0.01, term =="type"))$Category),
  Lifr_geno =  unique((signif_results_tukey_rna_esc_npc %>% filter( p.adj <= 0.01, term =="lifr_geno"))$Category)
) 
simplifyGOFromMultipleLists( lt = gsva_esc_npc_go_bp, 
                             db = 'org.Mm.eg.db',
                             ont = "BP"
                             )
...
gsva_plot
# ggsave(gsva_plot, filename = here("figures","gsva_plot.svg"), width = 6, height = 4, dpi = 300, units = "in")

```

### GSVA QTL table

```{r gsva_qtl_scans_esc_npc}
#| warning: false
#| message: false
#| cache: true

# note that I am mapping with the NPC samples from the ESC+NPC combined GSVA results
gsva_rna_esc_npc_results %>%
  filter(type =="npc") |> 
  select(Category, sampleid, Enrichment_Score) %>%
  pivot_wider( id_cols = sampleid , names_from = Category, values_from = Enrichment_Score) %>%
  column_to_rownames("sampleid") %>%
  as.matrix() -> gsva_npc_results_mat

# rankZ
gsva_npc_results_mat_rankZ <- apply(gsva_npc_results_mat, 2,rankZ )

shared.probs.npc_rna <- probs.npc_rna[ind=shared.samples]
shared.kinship.npc_rna <- calc_kinship(shared.probs.npc_rna, type = "loco")
#qtl mapping
gsva_qtl_npc <- scan1( genoprobs = shared.probs.npc_rna,
                   pheno = gsva_npc_results_mat_rankZ,
                   kinship = shared.kinship.npc_rna,
                   addcovar = covar.npc_rna[shared.samples,,drop=FALSE])


gsva_qtl_npc_peaks <- find_peaks( gsva_qtl_npc, threshold = 7, gmap)
# add interp_peak_bp, before, after
gsva_qtl_npc_peaks <- gsva_qtl_npc_peaks %>% 
  left_join( ., goannot_wdef %>% select(lodcolumn=GOID,TERM) %>% distinct()) %>% 
  mutate( TERM = ifelse( is.na(TERM), lodcolumn, TERM)) %>% 
  mutate(phenotype=lodcolumn) %>%
  mutate( peak_chr = chr,
...
gsva_rna_esc_npc_results %>%
  ...
  pivot_wider( id_cols = sampleid , names_from = Category, values_from = Enrichment_Score) %>%
  column_to_rownames("sampleid") %>%
  as.matrix() -> gsva_esc_results_mat

# rankZ
gsva_esc_results_mat_rankZ <- apply(gsva_esc_results_mat, 2,rankZ )

shared.probs.esc_rna <- probs.esc_rna[ind=shared.samples]
shared.kinship.esc_rna <- calc_kinship(shared.probs.esc_rna, type = "loco")
#qtl mapping
gsva_qtl_esc <- scan1( genoprobs = shared.probs.esc_rna,
                   pheno = gsva_esc_results_mat_rankZ,
                   kinship = shared.kinship.esc_rna,
                   addcovar = covar.esc_rna[shared.samples,,drop=FALSE])

gsva_qtl_esc_peaks <- find_peaks( gsva_qtl_esc, threshold = 7, gmap)
# add interp_peak_bp, before, after
gsva_qtl_esc_peaks <- gsva_qtl_esc_peaks %>% 
  left_join( ., goannot_wdef %>% select(lodcolumn=GOID,TERM) %>% distinct()) %>% 
  mutate( TERM = ifelse( is.na(TERM), lodcolumn, TERM)) %>% 
  mutate(phenotype=lodcolumn) %>%
  mutate( peak_chr = chr,
...
gsva_qtl_npc_peaks %>% 
  ...
  select( type, GOID = lodcolumn, TERM, peak_chr = chr, Lod = lod, peak_cM, peak_bp = interp_bp_peak) %>% 
  # filter( GOID %in% (signif_results_tukey_rna %>% filter( term %in% c("sex","lifr_geno"),
  #                                                          p.adj.signif != "ns" ))$Category ) %>%
  filter(Lod > 7.5) %>% 
  mutate_if(is.numeric, round, 2) %>% 
...
```{r closer_look_at_gsva_results_combined}
#| warning: false
#| message: false
#| eval: false
...
gsva_qtl_npc_peaks %>% 
  ...
  select( type, GOID = lodcolumn, TERM, peak_chr = chr, Lod = lod, peak_cM, peak_bp = interp_bp_peak) %>% 
  filter(
    (peak_chr == 1 & between(peak_bp, 104e06, 113e06)) |
    (peak_chr == 10 & between(peak_bp, 33e06, 40e06) ) |
    (peak_chr == 11 & between(peak_bp, 107e06, 111e06) )
  ) %>% 
  filter(Lod > 7) |> 
  create_dt()
...
outfile = ("/projects/munger-lab/projects/DO_mNPC/ESC_NPC_eQTL_comparison/_data/do_npc_esc_shared_samples_all_genes_model.hdf5")
...
library(MOFA2)
model <- load_model(here("_data","do_npc_esc_shared_samples_all_genes_model.hdf5"))
