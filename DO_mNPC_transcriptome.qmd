...
Further, there are genes uniquely expressed in each cell state which I functionally characterized using overrepresentation analysis (ORA).
...
for( i in 1:nrow(gsva_qtl_highlight)){
  ...
  qtl_effs <- scan1blup(genoprobs =probs.npc_rna[,peak_chr],
                   ...
                   addcovar = covar.npc_rna)
  cat('### ',gsub(":","",gsva_qtl_highlight$TERM[i]),'peak on', peak_chr ,' \n')
...
I can't seem to get WebGestaltR to work properly, keeps erroring out.
So instead I am doing ORA with up & down regulated genes.
