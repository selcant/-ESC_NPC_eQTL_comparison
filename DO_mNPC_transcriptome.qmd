...
I am starting with raw data, normalize using vst then removing both batch and sex effects. Then I use wilcoxon rank test to compare the two different cell states.
...
Here is the table of differentially expressed genes (FDR \<0.05, abs(log2foldchange) \> 0).
...
npc_esc_wilcox_df %>%
  filter( p <0.05, abs(log2foldchange) > 0.5) %>%
  left_join(all.genes) %>%
