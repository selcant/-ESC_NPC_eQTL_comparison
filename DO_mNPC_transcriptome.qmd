...
# doing vst on both data sets to remove the dependency of mean on variance
# I need to start with raw expression, normalize with vst then batch correct using combat.
expr.esc_rna_vst <- DESeq2::vst(object = round(raw.expr.esc_rna) )
expr.npc_rna_vst <- DESeq2::vst(object = round(raw.expr.npc_rna) )

dat.esc_rna <- log1p(expr.esc_rna_vst)
mod.esc_rna <- model.matrix(~ sex, data=covarTidy.esc_rna)
exprComBat.esc_rna <- ComBat(dat=dat.esc_rna, 
                             batch=covarTidy.esc_rna$libraryprep, 
                             mod=mod.esc_rna, 
                             par.prior=TRUE, 
                             prior.plots=FALSE)
expr.esc_rna_prep <- expm1(exprComBat.esc_rna)
expr.esc_rna_prep[expr.esc_rna_prep < 0] <- 0

dat.npc_rna <- log1p(expr.npc_rna_vst)
mod.npc_rna <- model.matrix(~ sex, data=covarTidy.npc_rna)
exprComBat.npc_rna <- ComBat(dat=dat.npc_rna, 
                             batch=covarTidy.npc_rna$libraryprep, 
                             mod=mod.npc_rna, 
                             par.prior=TRUE, 
                             prior.plots=FALSE)

expr.npc_rna_prep <- expm1(exprComBat.npc_rna)
expr.npc_rna_prep[expr.npc_rna_prep < 0] <- 0


expr.esc_rna <- t(expr.esc_rna_prep)
expr.npc_rna <- t(expr.npc_rna_prep)
```
...
After filtering out genes with low expression (median(tpm) < 0.5) and genes not expressed in at least half of the samples, we detect `r formatC(ncol(expr.esc_rna),big.mark = ",")` in ESCs and `r formatC(ncol(expr.npc_rna),big.mark = ",")` in NPCs, with `r formatC(length(shared.genes),big.mark = ",")` genes detected in both cell types.
...
ggscatter(var.npc_rna,
  ...
  stat_cor(method = "pearson", label.x = .9, label.y = 1.1) + # Add correlation coefficient
  xlab("Mean") +
  ylab("Variance") +
  theme_pubclean(base_size = 14) +
...
# ora_pc1_go <- enrichGO( gene = pc1_drivers$mgi_symbol,
#                         OrgDb = 'org.Mm.eg.db',
#                         keyType = "SYMBOL",
#                         pvalueCutoff = 0.05, # default 0.05
#                         pAdjustMethod = "BH", # default BH
#                         universe = npc.genes$mgi_symbol,
...
pc1_tfs <- g.pc1$result |> 
  filter( p_value <0.01, source == "TF") |> 
  mutate( TF = str_replace(str_replace(str_replace(str_to_upper(str_split_i(str_split_i(term_name, ":",2), ";",1)), pattern = "-", replacement = ""), pattern = ".",replacement = ""), pattern = "," , replacement = "")
    ) |> 
...
Let's try simplifying the GO BP terms:
...
simplifyGO( mat = GO_similarity( (g.pc1$result |> 
                                    filter( source =="GO:BP", p_value < 0.05))$term_id, 
                                 ont = "BP", 
                                 db = 'org.Mm.eg.db'))
...
-   TF motifs: `r paste( unique(pc1_tfs$TF), collapse = ", ")`
...
Let's simplify the GO BP terms from the two lists.
...
high_low_var_go_bp <- list(
  high_var = (g.high.var$result |> 
                filter( source == "GO:BP",
                        p_value < 0.05)),
  low_var = ((g.low.var$result |> 
                filter( source == "GO:BP",
                        p_value < 0.05)))
)
simplifyGOFromMultipleLists( lt = high_low_var_go_bp,  
...
simplifyGO( mat = GO_similarity( (npc_sex_ora$result |> 
                                    filter( source =="GO:BP", p_value < 0.05))$term_id, 
                                 db = 'org.Mm.eg.db',
                                 ont = "BP"))
...
## Principal component analysis with combined data
...
pca_esc_npc$x%>%
  ...
  labs( col = "Cell state", shape = "Sex")-> pca_plot

# ggsave(pca_plot, filename = here("figures","figure1c.svg"), width = 7, height = 4, dpi = 300, units = "in")
...
pca_esc_npc$x%>%
  ...
  labs( col = "Cell state", shape = "Sex") -> pca_plot_pc23

# combined_plot <- ggarrange( pca_plot, pca_plot_pc23, common.legend = T, nrow = 1, legend = "right")
# 
# ggsave(combined_plot, filename = here("figures","figure1c_combined.svg"), width = 10, height = 4, dpi = 300, units = "in")
...
g.pc1_full <- gost(
  query = pc1_drivers_full$mgi_symbol,
  organism = "mmusculus",
  domain_scope = "custom",
  custom_bg = (all.genes |> filter(ensembl_gene_id %in% shared.genes))$mgi_symbol,
  evcodes = TRUE,
...
g.pc1_full$result <- g.pc1_full$result %>% filter(term_size < 660)
...
pc2_drivers <- pca_esc_npc$rotation[,"PC2", drop = FALSE] %>% 
  as_tibble( rownames = "ensembl_gene_id") %>% 
  left_join( all.genes %>%  
               select(ensembl_gene_id, mgi_symbol)
             ) %>% 
...
pc3_drivers <- pca_esc_npc$rotation[,"PC3", drop = FALSE] %>% 
  as_tibble( rownames = "ensembl_gene_id") %>% 
  left_join( all.genes %>%  
               select(ensembl_gene_id, mgi_symbol)
             ) %>% 
...
pc4_drivers <- pca_esc_npc$rotation[,"PC4", drop = FALSE] %>% 
  as_tibble( rownames = "ensembl_gene_id") %>% 
  left_join( all.genes %>%  
               select(ensembl_gene_id, mgi_symbol)
             ) %>% 
...
# pca_esc_npc$rotation[,"PC4", drop = FALSE] %>% 
#   as_tibble( rownames = "ensembl_gene_id") %>% 
#   left_join( all.genes %>%  
#                select(ensembl_gene_id, mgi_symbol, gene_chr)
#              ) %>% 
#   filter( abs(PC4) >= quantile(abs(PC4), 0.95)) |> 
#   arrange(desc(abs(PC4)))
#   count(gene_chr)

pc1_tfs <- g.pc1_full$result |> 
  filter( p_value <0.01, source == "TF") |> 
  mutate( TF = str_replace(str_replace(str_replace(str_to_upper(str_split_i(str_split_i(term_name, ":",2), ";",1)), pattern = "-", replacement = ""), pattern = ".",replacement = ""), pattern = "," , replacement = "")
    ) |> 
  mutate( TF = str_to_title(TF))

pc2_tfs <- g.pc2$result |> 
  filter( p_value <0.01, source == "TF") |> 
  mutate( TF = str_replace(str_replace(str_replace(str_to_upper(str_split_i(str_split_i(term_name, ":",2), ";",1)), pattern = "-", replacement = ""), pattern = ".",replacement = ""), pattern = "," , replacement = "")
    ) |> 
  mutate( TF = str_to_title(TF))

pc3_tfs <- g.pc3$result |> 
  filter( p_value <0.01, source == "TF") |> 
  mutate( TF = str_replace(str_replace(str_replace(str_to_upper(str_split_i(str_split_i(term_name, ":",2), ";",1)), pattern = "-", replacement = ""), pattern = ".",replacement = ""), pattern = "," , replacement = "")
    ) |> 
  mutate( TF = str_to_title(TF))
...
Let's simplify the GO BP terms from the three PC lists.

```{r esc_npc_pc123_go_bp}
#| warning: false
#| message: false
#| fig-width: 21
#| fig-height: 8

combined_pca_go_bp <- list(
  PC1 = (g.pc1_full$result |> 
                filter( source == "GO:BP",
                        p_value < 0.05)),
  PC2 = (g.pc2$result |> 
                filter( source == "GO:BP",
...
simplifyGOFromMultipleLists( lt = combined_pca_go_bp,  
                             go_id_column = "term_id",
                             padj_column = "p_value",
                             padj_cutoff = 0.05, 
                             db = 'org.Mm.eg.db',
...
-   TFs: `r paste( unique(pc1_tfs$TF), collapse = ", ")`

-   protein ubiquitination, ubiquitin ligase complex, ubiquitin-protein transferase activity, modification-dependent protein catabolic process, ubiquitin-dependent protein catabolic process, ubiquitin-like protein transferase activity, protein polyubiquitination, Ubiquitin mediated proteolysis

-   Nervous system development, Axon guidance, dendritic tree, central nervous system development, presynapse, cell morphogenesis involved in neuron differentiation, postsynapse

-   embryonic morphogenesis, chordate embryonic development, embryo development ending in birth or egg hatching

-   Signaling:

    -   Signaling pathways regulating pluripotency of stem cells

    -   GPCR sigaling: Signaling by GPCR, G alpha (12/13) signalling events, GPCR downstream signalling, adenylate cyclase-modulating G protein-coupled receptor signaling pathway, G protein-coupled receptor signaling pathway

    -   PI3K/AKT Signaling: Negative regulation of the PI3K/AKT network, PI5P, PP2A and IER3 Regulate PI3K/AKT Signaling, heterotrimeric G-protein complex

    -   Toll-like receptor signaling pathway

    -   regulation of MAPK cascade, ERK5 cascade

PC2:

-   TFs: `r paste( unique(pc2_tfs$TF), collapse = ", ")`

-   Cell cycle and more specifically the mitotic assembly checkpoint.

-   Mitochondrial functions like: cellular respiration, TCA cycle.


PC3:

-   TFs: `r paste( unique(pc3_tfs$TF), collapse = ", ")`
...
PC4 is mostly driven by genes on sex chromosomes (145/605 drivers are on X chr).

```{r pc4_drivers}
#| eval: false
pc4_drivers |> 
  left_join( all.genes |> 
               select(ensembl_gene_id, gene_chr)) |> 
  count(gene_chr) |> 
  arrange(desc(n))
...
save(sample_cor, file = here("_data","npc_esc_rna_sample_cor_perm_pearson_vst.RData"))
```
...
load(here("_data","npc_esc_rna_sample_cor_perm_pearson_vst.RData"))
...
```{r}
#| label: mapping_with_cor
#| eval: false

real_sample_cor_dist |> 
  select(esc, cor_expr) |> 
  mutate( esc =gsub(".esc_rna","",esc)) |> 
  column_to_rownames("esc") |> 
  as.matrix()-> real_sample_cor_mt
real_sample_cor_mt_rankZ <- apply(real_sample_cor_mt, 2, rankZ)

cor_scan <- scan1( genoprobs = shared.probs.esc_rna, 
                   pheno =real_sample_cor_mt_rankZ, 
                   kinship = kinship.shared.esc_rna, 
                   addcovar = covar.esc_rna[shared.samples, "sex"] )

cor_scan_perm <- scan1perm(genoprobs = shared.probs.esc_rna, 
                   pheno =real_sample_cor_mt_rankZ, 
                   kinship = kinship.shared.esc_rna, 
                   addcovar = covar.esc_rna[shared.samples, "sex"] ,
                   n_perm = 100)
plot_scan1(cor_scan, pmap)
find_peaks(cor_scan, pmap, threshold = 6)
```
...
sample_cor_df %>% 
  ...
  geom_text( aes( label = paste0("median =\n",round(median(cor_expr),2)),
                  x = median(cor_expr)-0.05, y = 19), size = 6
             )+
  theme_pubclean(base_size = 20)+
  color_palette("npg")+
  fill_palette("npg")+
  ...
  ylim(0,20)+
  xlab("Correlation")+
  ylab("# of samples") -> sample_hist

# ggsave(sample_hist, filename = here("figures","sample_cor.svg"), width = 5, height = 4, dpi = 300, units = "in")
```
...
save(gene_cor_df, file = here("_data","gene_cor_df_vst.RData"))
...
load(here("_data","gene_cor_df_vst.RData"))
...
Let's simplify the GO BP terms from the two unique gene lists:

```{r esc_npc_uniq_go_bp}
#| warning: false
#| message: false
#| fig-width: 21
#| fig-height: 8

esc_npc_uniq_go_bp <- list(
  ESC = (esc_specific_ora$result |> 
                filter( source == "GO:BP",
                        p_value < 0.05)),
  NPC = (npc_specific_ora$result |> 
                filter( source == "GO:BP",
                        p_value < 0.05))
)
simplifyGOFromMultipleLists( lt = esc_npc_uniq_go_bp,  
                             go_id_column = "term_id",
                             padj_column = "p_value",
                             padj_cutoff = 0.05, 
                             db = 'org.Mm.eg.db',
...
I am starting with raw data, normalize using vst/UQ then removing both batch and sex effects. Then I use wilcoxon rank test to compare the two different cell states.


```{r wilcoxon_rank_test_prep_w_vst}
#| warning: false
#| message: false
#| eval: false
...
dat.esc_rna <- log1p(expr.esc_rna_vst)
mod.esc_rna <- model.matrix(~ sex, data=covarTidy.esc_rna)
exprComBat.esc_rna_batch <- ComBat(dat=dat.esc_rna, 
                             batch=covarTidy.esc_rna$libraryprep, 
                             mod=mod.esc_rna, 
                             par.prior=TRUE, 
                             prior.plots=FALSE)
exprComBat.esc_rna_sex <- ComBat( dat=exprComBat.esc_rna_batch, 
                             batch=covarTidy.esc_rna$sex, 
                             par.prior=TRUE, 
                             prior.plots=FALSE)

expr.esc_rna_prep <- expm1(exprComBat.esc_rna_sex)
expr.esc_rna_prep[expr.esc_rna_prep < 0] <- 0

dat.npc_rna <- log1p(expr.npc_rna_vst)
mod.npc_rna <- model.matrix(~ sex, data=covarTidy.npc_rna)
exprComBat.npc_rna_batch <- ComBat(dat=dat.npc_rna, 
                             batch=covarTidy.npc_rna$libraryprep, 
                             mod=mod.npc_rna, 
                             par.prior=TRUE, 
                             prior.plots=FALSE)
exprComBat.npc_rna_sex <- ComBat(dat=exprComBat.npc_rna_batch, 
                             batch=covarTidy.npc_rna$sex,
                             par.prior=TRUE, 
                             prior.plots=FALSE)


expr.npc_rna_prep <- expm1(exprComBat.npc_rna_sex)
expr.npc_rna_prep[expr.npc_rna_prep < 0] <- 0


expr.esc_rna_state_only <- t(expr.esc_rna_prep)
expr.npc_rna_state_only <- t(expr.npc_rna_prep)

expr.npc_rna_state_only %>%
  as_tibble( rownames = "sampleid") %>%
  pivot_longer( colnames(expr.npc_rna_state_only), names_to = "ensembl_gene_id", values_to = "npc") %>%
  full_join(
    expr.esc_rna_state_only %>%
      as_tibble( rownames = "sampleid") %>%
      pivot_longer( colnames(expr.esc_rna_state_only), names_to = "ensembl_gene_id", values_to = "esc")
  ) %>%
...
npc_esc_expr_df %>%
  ...
  group_by(ensembl_gene_id) %>%
  rstatix::wilcox_test(., expr~cell_type, paired = TRUE, p.adjust.method = "BH") -> npc_esc_wilcox_p
...
```{r wilcoxon_rank_test_prep_w_uq}
#| warning: false
#| message: false
#| eval: false

esc_rna_q75 <- apply(raw.expr.esc_rna, 2, quantile, 0.75)
esc_rna_ratio <- mean(esc_rna_q75)/esc_rna_q75
expr.esc_rna_uq <- sweep(raw.expr.esc_rna, 2, esc_rna_ratio, FUN="*")

npc_rna_q75 <- apply(raw.expr.npc_rna, 2, quantile, 0.75)
npc_rna_ratio <- mean(npc_rna_q75)/npc_rna_q75
expr.npc_rna_uq <- sweep(raw.expr.npc_rna, 2, npc_rna_ratio, FUN="*")

dat.esc_rna <- log1p(expr.esc_rna_uq)
mod.esc_rna <- model.matrix(~ sex, data=covarTidy.esc_rna)
exprComBat.esc_rna_batch <- ComBat(dat=dat.esc_rna, 
                             batch=covarTidy.esc_rna$libraryprep, 
                             mod=mod.esc_rna, 
...
dat.npc_rna <- log1p(expr.npc_rna_uq)
mod.npc_rna <- model.matrix(~ sex, data=covarTidy.npc_rna)
exprComBat.npc_rna_batch <- ComBat(dat=dat.npc_rna, 
                             batch=covarTidy.npc_rna$libraryprep, 
                             mod=mod.npc_rna, 
...
ontology_ids <- ontology_genes |> select(gs_name, gs_exact_source)
...
Let's simplify the GO terms, the others don't return that many:

```{r esc_npc_degenes_simplifygo}
#| warning: false
#| message: false
#| fig-width: 16
#| fig-height: 8

degenes_goids <- ontology_res |>
  filter( str_detect(pathway, "GOBP"), padj < 0.05) |>
  left_join( ontology_ids, by = c("pathway"="gs_name"))

simplifyGO( mat = GO_similarity( degenes_goids$gs_exact_source,
                                  db = 'org.Mm.eg.db',
                                 ont = "BP")
            )
...
<!-- npc_esc_wilcox_df %>% -->
<!--   filter( p < 0.05, (log2foldchange) > .5) %>% -->
